...
**FR-WALLET-2.** Calls are billed at a **flat per-client rate per minute** (default $0.30/min), **rounded up to the next started minute** — 61 seconds bills as 2 minutes. A completed call with zero duration bills nothing; failed/busy/no-answer calls bill nothing. The rate and the round-up rule are stated plainly on the client's wallet screen.
...
**FR-WALLET-5.** **Test calls are billed like any call** — same rate, same round-up, same reserve-then-settle — and are attributed in the ledger. (Supersedes their silent Becca-absorption; the test screen's "billed like any call" is now true.)
